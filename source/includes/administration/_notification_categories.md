...
curl "https://portal.coxedge.com/api/v2/content/categories?language=:language&organization_id=:organizationId" \
...
```shell
# Retrieve notification category
curl "https://portal.coxedge.com/api/v2/content/categories/:id?language=:language" \
   -H "MC-Api-Key: your_api_key"
```
> The above command returns JSON structured like this:
...
```shell
# Create notification category
curl -X POST "https://portal.coxedge.com/api/v2/content/categories/" \
   -H "MC-Api-Key: your_api_key" \
...
```shell
# Update notification category
curl -X PUT "https://portal.coxedge.com/api/v2/content/categories/:id" \
   -H "MC-Api-Key: your_api_key" \
   -H "Content-Type: application/json" \
   -d "request-body"
```
