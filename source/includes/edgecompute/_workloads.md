...
curl -X GET \
   ...
   "https://portal.coxedge.com/api/v2/services/edge-services/test-area/workloads"
```
...
`userData`<br/>*string* | User data compatible with cloud-init to be run on the workload's instances.
...
curl -X GET \
   -H "MC-Api-Key: your_api_key" \
   "https://portal.coxedge.com/api/v2/services/edge-services/test-area/workloads/1b932678-1038-4ab4-9fa4-c4c06e696e20"
```
...
`userData`<br/>*string* | User data compatible with cloud-init to be run on the workload's instances. This should only be set for a VM-based workload.
